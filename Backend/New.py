...
def build_where(filters: Dict[str, Any], start_dt: pd.Timestamp | None, end_dt: pd.Timestamp | None, date_col: str) -> Tuple[List[str], List[Any]]:
    where: List[str] = []
    params: List[Any] = []
...
# ---------------------------------------------------------------------
# Metric keyword helper (NEW)
# ---------------------------------------------------------------------

def extract_metric_phrase(question: str) -> str | None:
    m = re.search(
        r"\b(with|for)\s+(.+?)(?:\s+between|\s+from|\s+in|\s+on|\s*$)",
        question,
        re.IGNORECASE,
    )
    if not m:
...
def add_metric_keywords_where(question: str, where_clauses: List[str], params: List[Any]) -> None:
    metric_phrase = extract_metric_phrase(question)
    if not metric_phrase:
        return

    words = re.findall(r"[A-Za-z]+", metric_phrase.lower())
    stop = {"the", "and", "or", "of", "in", "on", "to", "for", "with", "records", "record"}
    words = [w for w in words if len(w) >= 3 and w not in stop]
    words = words[:4]

    for w in words:
        where_clauses.append("dataelement_name ILIKE %s")
        params.append(f"%{w}%")
...
def answer_question(question: str, debug: bool = False) -> dict:
    ...
    if not conn_str:
        raise ValueError("Missing DB config. Set DHIS2_DSN or PGHOST/PGPORT/PGDATABASE/PGUSER/PGPASSWORD.")
...
""".strip()
    ...
    if not use_llm_sql:
        ...
        # NEW: add metric keyword filters
        add_metric_keywords_where(question, where_clauses, params)
        ...
        # Even in LLM mode, add extra metric filters to improve accuracy (MVP)
        where_clauses = [where_sql] if where_sql.strip() else []
        add_metric_keywords_where(question, where_clauses, params)
        where_sql = " AND ".join(where_clauses) if where_clauses else "TRUE"

        ok, err = validate_llm_where_sql(where_sql, params)
        if not ok:
            # rollback if adding metric keywords made it unsafe
            where_sql = instr.get("where_sql", "TRUE")
    ...
    if summary_mode:
        ...
        clean_df = df.copy()
        clean_df = clean_df.rename(columns={"total_value": "Total", "record_count": "Records"})
    else:
